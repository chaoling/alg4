public class Board {
    ...
    public Board(int[][] blocks) {        
        ...
        mDim = blocks.length;
        int N = mDim * mDim;
        //System.out.println("Dim ="+mDim);
        assert mDim > 0; //square blocks
        mBlks = new short[N];
        for (int i = 0; i < N; i++) {
            mBlks[i] = (short) blocks[getRow(i)][getCol(i)];
        }
    }
    ...
    public int manhattan() {               
        ...
        if (manhattandist == -1) {
            ...
            for (int i = 0; i < mDim * mDim; i++) {
                if (!(mBlks[i] == 0 
                          || mBlks[i] == (i + 1) % N)) {
                    res += mandist(getRow(i), getCol(i), mBlks[i]);
                }
            }
        } else {
            res = manhattandist;
    ...
    public Board twin() {
        ...
        for (int i = 0; i < mDim * mDim; i++) {
            ...
            if (!swapDone && col > 0 
                    && twinBlks[i] != 0 
                    && twinBlks[i - 1] != 0) {
                swap(twinBlks, row, col, row, col-1);
    ...
    public String toString() {
        ...
        for (int i = 0; i < mDim; i++) {
            for (int j = 0; j < mDim; j++) {
                sb.append(String.format("%2d ", mBlks[i * mDim + j]));
            }
            sb.append("\n");
        }
        return sb.toString();
    ...
    private void swap(short[] tiles, int x0, int y0, int x1, int y1) {
        short tmp = tiles[x0 * mDim + y0];
        tiles[x0 * mDim + y0] = tiles[x1 * mDim + y1];
        tiles[x1 * mDim + y1] = tmp;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("block size: "+blocks.length);
