...
public class Solver {
    ...
    private SearchNode mTwinRootNode;
    ...
    public Solver(Board initial) {
        ...
        mTwinRootNode = new SearchNode(initial.twin(),0);
        //mTwinRootNode.setIsTwin(true);
        mPQ.insert(mRootNode);
        mPQ.insert(mTwinRootNode);
    }
    
    public boolean isSolvable() {
        // is the initial board solvable?
        solution();
        //System.out.println("solution: "+mSolution.peek());
        if (mSolution.peek().equals(mRootNode.getBoard())) {
            return true;
        } else {
            //twin rootnode solvable
            return false;
        }
    }
