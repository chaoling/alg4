    ...
    public  double energy(int x, int y) {
        ...
        if (x < 0 || x >= width() || y < 0 || y >= height()) {
            throw new IndexOutOfBoundsException(
                  "point("+x+","+y+") is out of pic bounds");
        }
...
//        mPic = iTranspose(mPic);
//        transposed = false;
        int[] res = null;
        this.mSp = new ShortestEnergyPath(width(), height(), false);
        List<Integer> endPts = new ArrayList<Integer>();
        for (int row = 0; row < height(); row++) {
            endPts.add(xyTop(width() - 1, row));
        }
        int minEndPoint = findMin(this.mSp, endPts);
    ...
    public   int[] findVerticalSeam() {
        ...
        int[] res = null;
        this.mSp = new ShortestEnergyPath(width(), height(), true);
        List<Integer> endPts = new ArrayList<Integer>();
        for (int col = 0; col < width(); col++) {
            endPts.add(xyTop(col, height() - 1));
        }
        int minEndPoint = findMin(this.mSp, endPts);
    ...
    public ShortestEnergyPath(int width, int height, boolean vertical) {
        ...
        List<Integer> startPts = new ArrayList<Integer>(); 
        //initial starting points to search for in sp algorithm
        if (isVertical) {
            //start from the top row
            for (int col = 0; col < width; col++) {
                startPts.add(xyTop(col, 0));
            }
        } else {
            //start from the left column
            for (int row = height - 1; row >= 0; row--) {
                //!!!!!!! pay attention to the starting pts in horizontal
                // i.e., skip columns
                startPts.add(xyTop(0, row));
            }
        }
     ...
     * @param v the destination vertex
     * @return <tt>true</tt> if there is a 
     ...
     * <tt>s</tt> to vertex <tt>v</tt>
     *    as an iterable of edges, and <tt>null</tt> 
     * if no such path
     */
    public Iterable<Integer> pathTo(int v) {
        if (!hasPathTo(v)) return null;
        Stack<Integer> path = new Stack<Integer>();
        for (int p = pathTo[v]; p != -1; p = pathTo[p]) {
            path.push(p);
        }
        return path;
    }
    ...
    private int[] verticalPath(int endPoint, ShortestEnergyPath sp) {
     ...
     private Picture transpose(Picture p) {
         int width = p.width(); //cols
         int height = p.height(); //rows
         Picture np = new Picture(height, width);
         for (int col = 0; col < width(); col++) {
             for (int row = 0; row < height(); row++) {
                int newcol = height - row -1;
                int newrow = col;
                np.set(newcol, newrow, p.get(col, row));
             }
         }
         return np;
     }
     
     private Picture iTranspose(Picture p) {
         int width = p.width(); //cols
         int height = p.height(); //rows
         Picture np = new Picture(height, width);
         for (int col = 0; col < width(); col++) {
    ...
    private int[] pToxy(int p) {
        if (p < 0 || p >= width() * height()) {
            throw new IllegalArgumentException("point is out of the graph");
        } else {
            int y = p / width();
            int x = p - y * width();
            return new int[] {x, y};
